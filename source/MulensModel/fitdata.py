    ...
    def _set_data_magnification_curves(self, bad=True):
        ...
        if self._model.n_sources == 1:
            self._data_magnification_curve = \
                self._model.get_magnification_curve(time=self._dataset.time[select], magnification_setup=self._dataset.magnification_setup ,**magnification_kwargs)
        elif self._model.n_sources >= 2:
            self._data_magnification_curves = self._model.get_magnification_curves(
                        time=self._dataset.time[select], magnification_setup=self._dataset.magnification_setup, **magnification_kwargs)
